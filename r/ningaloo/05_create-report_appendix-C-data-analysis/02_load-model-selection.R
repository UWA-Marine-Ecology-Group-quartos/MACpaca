...
# `status` is omitted - the heatmap colours a term by the SIGN of its Spearman
# correlation with the response, which is undefined for an unordered factor.
...
# Row order in Table C1.1 / C1.2 and on the heatmaps. Only sand, sessile
# invertebrates and reef were modelled for Ningaloo - macroalgae, rock and
# seagrasses never entered the FSS loop (>80% zeros).
...
# Pretty "detrended+aspect+Z" string for the Model column
  ...
  purrr::imap_dfr(final_models, function(mod, resp) {
    ...
    if (!any(rows$selected)) {
      warning("The final model for '", resp, "' (", paste(final_set, collapse = "+"),
              ...
              "the status term appears in the FSS modnames the way expected below.")
    }
...
get_habitat_report_table <- function(models = NULL) {
  ...
  mark_selected(candidates, models) %>%
    ...
    dplyr::select(-aicc)   # Table C1.1 does not report raw AICc
}
...
get_fish_report_table <- function(models = NULL) {
  ...
  # `status` is forced into every fish candidate via null.terms in
  # 06_model-data_fish.R (never `year` - fish years are pooled for Ningaloo).
