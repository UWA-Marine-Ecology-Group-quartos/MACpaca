controlplot_fish <- function(data, metric, amp_abbrv, state_abbrv,
  ...
  if (metric == "cti") {
    ...
    p <- ggplot() +
      ...
      theme_classic() +
      scale_x_continuous(breaks = c(2018, 2025)) + # TODO set to your survey years
      coord_cartesian(xlim = c(2015, 2026)) + # TODO set to your survey years
    ...
    p <- ggplot(
      ...
      theme_classic() +
      scale_x_continuous(breaks = c(2018, 2025)) + # TODO set to your survey years
      coord_cartesian(xlim = c(2015, 2026), ylim = c(0, NA)) + # TODO set to your survey years
      scale_fill_manual(values = fill_vals, name = "Marine Parks", drop = FALSE) +
      scale_shape_manual(values = shape_vals, name = "Marine Parks", drop = FALSE) +
      labs(
        x = "Year",
