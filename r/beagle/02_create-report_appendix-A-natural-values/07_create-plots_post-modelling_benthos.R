...
# TODO Plot extent
prediction_limits <- c(146.4226, 147.6526, -39.6899, -38.9574) #changed already
...
control_all <- purrr::map(years, \(yy) {
  ...
  controldata_benthos(dat = dat_yy, year = yy, amp_abbrv = "BMP", state_abbrv = "KGMP") # TODO set park abbreviations
})
...
for (taxa_code in names(taxa_lookup)) {
  ...
  p_taxa <- controlplot_benthos(
    ...
    taxa = taxa_code,
    amp_abbrv = "BMP", # TODO set park abbreviations
    state_abbrv = "KGMP",
...
hab_fills <- scale_fill_manual(
  name = "Habitat",
  limits = c("Sessile invertebrates", "Macroalgae",  "Sediment"), #"Bare Rock","Seagrass",
  values = c(
    #"Bare Rock" = "grey40",
    "Sessile invertebrates" = "plum",
    "Macroalgae" = "darkgoldenrod4",
...
site_limits <- c(146.41, 147.58, -39.639, -38.98) # TODO set limits
