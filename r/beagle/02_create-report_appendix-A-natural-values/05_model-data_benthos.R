...
model_dat <- habi %>%
  pivot_longer(cols = c(macroalgae, sand,  sessile_invertebrates, reef), #rock, , seagrasses
...
for(i in 1:length(resp.vars)){
  ...
  for(m in 1:nrow(out.i)){
    ...
    if(!is.na(best.model.name) && best.model.name != "null"){
...
marine_parks <- st_read("data/amp_shapefile/Australian_Marine_Parks_v2.shp") %>%
  dplyr::filter(name %in% c("Beagle")) %>%  # TODO select marine parks in your area
...
resp.vars <- c("p_sand", "p_inverts", "p_reef") #"p_macro", "p_seagrass",  "p_rock",)
...
for (this_label in pred.labels) {
  ...
  se_rasts <- terra::subset(
    preddf_m,
    c("p_sand.se.fit", "p_inverts.se.fit", "p_reef.se.fit") #"p_macro.se.fit", "p_rock.se.fit", "p_seagrass.se.fit",
  )
  ...
  pred_dom_df <- as.data.frame(dom_rast, xy = TRUE, na.rm = TRUE) %>%
    dplyr::mutate(
      ...
      dom_tag = factor(
        dom_tag,
        levels = c("Sand", "Sessile invertebrates", "Reef") #"Rock", "Macroalgae", "Seagrass",
      )
    )
