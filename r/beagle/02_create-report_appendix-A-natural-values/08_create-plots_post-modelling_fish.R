...
prediction_limits <- c(146.4226, 147.6526, -39.6899, -38.9574) #already been changed
...
control_all <- purrr::map(years, \(yy) {
  ...
  controldata_fish(dat = dat_yy, year = yy, amp_abbrv = "BMP", state_abbrv = "KGNP") # TODO park abbreviations
})
...
for (metric_code in names(metric_lookup)) {
  ...
  p_metric <- controlplot_fish(
    ...
    metric = metric_code,
    amp_abbrv = "BMP", # TODO park abbreviations
    state_abbrv = "KGNP",
