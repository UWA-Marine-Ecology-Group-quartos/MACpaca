...
#
# The same model is also fitted to legal sized males and legal sized females
# separately. The sexes differ in behaviour and in how the fishery takes them,
# so a change in legal catch can be driven by one sex alone; the combined legal
# model cannot show that. Note these are not independent tests - legal male and
# legal female catch together make up the legal catch already modelled above.
#
...
models <- list(
  legal        = fit_catch("n_legal"),
  sublegal     = fit_catch("n_sublegal"),
  legal_male   = fit_catch("n_legal_male"),
  legal_female = fit_catch("n_legal_female")
)
...
cat("\nNOTE: the legal male and legal female models are subsets of the legal\n")
cat("model, not independent tests of it.\n")
if (!use_site) {
...
ggplot(plot_data, aes(x = year, y = mean, fill = status, group = status)) +
  ...
  facet_wrap(~size_class, ncol = 2, scales = "free_y") +
...
ggplot(plot_data, aes(x = factor(year), y = mean, fill = status)) +
...
ggsave(file.path(plot_dir, paste0(name, "_lobster-glm-barplot.png")),
       height = 4.5, width = 11, dpi = 300, bg = "white")

# Box plot of the raw per-pot data ---------------------------------------------

raw_long <- model_data %>%
  tidyr::pivot_longer(paste0("n_", names(class_labels)),
                      names_to = "size_class", values_to = "count") %>%
  dplyr::mutate(size_class = factor(class_labels[sub("^n_", "", size_class)],
                                    levels = class_labels))
