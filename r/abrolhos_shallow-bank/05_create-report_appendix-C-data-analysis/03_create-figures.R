...
# WHICH YEAR/STATUS DO THE CURVES SHOW? ---------------------------------------
...
# =============================================================================
# Model selection did not keep the same shape for every response - some
# habitat smooths are fitted separately by year (sand, macroalgae, reef),
# others by zone status (kelp, rock, sessile invertebrates), and none of the
# four fish models keep a year-varying smooth or a status term at all.
...
ggsave(file.path(figdir, paste0(name, "_habitat-importance.png")), fig_c1_1,
       width = 7, height = 6, dpi = 300, bg = "white")     # 6 habitat responses
...
# combine_benthos is FALSE for this park - year is a covariate in the sand,
# macroalgae and reef models, so the curves genuinely differ by survey year.
...
# Responses whose selected model has no `by = year` smooth (kelp, rock,
# sessile invertebrates - all fitted `by = status` instead). Their panels are
# the same shape in both year figures, offset only by the parametric year
# effect where present. That is a real result, not a plotting bug, but it is
# worth knowing before someone asks why two years look identical.
...
# ---- Fish: modelled count metric, single figure -----------------------------
# None of the four final fish models carry a by = year smooth (or a status
# term at all), so the curve shape is identical across years - one figure is
# enough, held at the first survey year (see factor_ref).
...
if (length(fish_flat_across_years)) {
  message("Fish responses with no year-varying smooth (identical curve ",
          "shape across survey years, intercept aside): ",
