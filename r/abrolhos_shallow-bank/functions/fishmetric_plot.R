fishmetric_plot <- function(metric_name,
  ...
  # ---- State marine parks are optional ----
  ...
  # Single pooled surface: prediction and SE are placed side by side, titled
  # rather than row-labelled. Multiple years keep the original stacked layout,
  ...
  # ---- Row labels (stacked layout only) ----
  ...
  p_out <- p_out +
    ...
    theme(
      ...
      legend.box.margin = margin(2, 2, 2, 4),
      panel.spacing     = unit(0.5, "mm"),
      plot.margin       = margin(2, 2, 2, 2, unit = "mm")
    )

  p_out <- cowplot::plot_grid(p_out, marine_park_legend(wasanc = wasanc),
