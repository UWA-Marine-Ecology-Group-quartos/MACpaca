...
# Species richness
m_richness <- gam(count ~ s(geoscience_roughness, k = 3, bs = "cr"),
                  data = fabund %>% dplyr::filter(response %in% "species_richness"),
                  family = gaussian(link = "identity"))
summary(m_richness)

# CTI
m_cti <- gam(count ~ s(geoscience_roughness, k = 3, bs = "cr"),
             data = fabund %>% dplyr::filter(response %in% "cti"),
             family = gaussian(link = "identity"))
summary(m_cti)

# B20
# NOTE the null model was top-ranked (delta AICc 0); aspect is the best
# non-null model at delta 0.926 and is forced in here so B20 can be predicted
# spatially. Deviance explained is only 6.8% - treat the surface as indicative
m_b20 <- gam(count ~ s(geoscience_aspect, k = 3, bs = "cc"),
...
prasts_2021 <- rast(
  predicted_fish %>%
    dplyr::filter(as.character(year) %in% "2021") %>%
