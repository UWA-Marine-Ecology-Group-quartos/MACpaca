...
# Rock - not modelled. Null model is within delta AICc 2 (1.611), so no
# predictor is supported. Rock is 0-2 points out of ~30 in most samples.
# m_rock <- gam(cbind(rock, total_pts - rock) ~
#                 s(geoscience_depth, k = 3, bs = "cr"),
#               data = habi, method = "REML", family = binomial("logit"))
# summary(m_rock)

# Macroalgae - not modelled, >80% zeros
# m_macro <- gam(...)

# Seagrass - not modelled, absent from all samples
# m_seagrass <- gam(...)
