...
message("Dropping incomplete years: ",
...
for (metric_name in names(fish_metric_lookup)) {
  ...
  p_metric <- fishmetric_plot(
    ...
    pred_limits = NULL,
    se_limits = NULL,
    wasanc = NULL        # or wasanc = wasanc for parks with state reserves
  )
  print(p_metric)
...
control_all <- purrr::map(years, \(yy) {
  ...
  controldata_fish(dat = dat_yy, year = yy, amp_abbrv = "ABMP")   # TODO park abbreviation
})
