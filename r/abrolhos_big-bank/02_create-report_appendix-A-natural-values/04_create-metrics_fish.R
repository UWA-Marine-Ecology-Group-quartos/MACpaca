...
# Samples that actually have length data - BRUV only, and only deployments with
# rows in the length file. Treating BOSS or unmeasured deployments as zero
# biomass would drag every species mean down.
b20_samples <- readRDS(paste0("data/", park, "/raw/_length-with-zeros.RDS")) %>%
  dplyr::distinct(campaignid, sample) %>%
  dplyr::left_join(metadata %>% dplyr::select(campaignid, sample, year),
...
count.wide <- count %>%
  ...
  mutate(
    Year = case_when(
      grepl("^2021", campaignid) ~ "2021", # TODO adjust to years in data
      TRUE ~ campaignid
    )
  ) %>%
  left_join(
...
all_samples <- b20_samples %>% distinct(year, sample)
...
marine_parks_amp <- st_read("data/south-west network/spatial/shapefiles/western-australia_marine-parks-all.shp") %>%
  dplyr::filter(name %in% c("Abrolhos")) %>%
...
b20_metadata <- b20_samples %>% distinct(year, sample) %>% glimpse()
