...
# Final models - TODO these must match whatever is chosen in
# 05_model-data_benthos.R once that has actually been run (always choose the
# simplest model within delta AICc <= 2)
# Rock, Macroalgae and Seagrass are not modelled - all three came back at
# >80% zeros in 05_model-data_benthos.R's response-vector check, so they
# never entered the FSS loop and have no candidate rows to select from here.
# Sand
...
# Guard
if (!combine_benthos &&
    !all(vapply(list(m_sand, m_inverts, m_reef),
...
# Fish
# ------------------------------------------------------------
fabund <- dplyr::bind_rows(
  readRDS(here("data", park, "tidy", paste0(name, "_tidy-count.rds"))),
  readRDS(here("data", park, "tidy", paste0(name, "_tidy-b20.rds")))
) %>%
  dplyr::mutate(year   = droplevels(factor(as.character(year), levels = year_levels)),
                status = droplevels(factor(as.character(status)))) %>%
  glimpse()
...
# Final models - TODO these must match whatever is chosen in
# 06_model-data_fish.R once that has actually been run (always choose the
# simplest model within delta AICc <= 2). year and status are forced into
# every model as null.terms there, so both are always kept here too.
# Total abundance
...
if (!all(vapply(list(m_abundance, m_richness, m_cti, m_b20),
                function(m) all(c("year", "status") %in% all.vars(formula(m))),
                logical(1)))) {
  stop("A final fish model is missing the forced year/status term.")
}

# Save - named to match the `response` column values used throughout
# fabund/the FSS CSVs (species_richness/total_abundance), not the shorter
# habitat-style names.
