...
# Fish
# ------------------------------------------------------------
models_fish <- load_final_models_fish()
fish_dat    <- load_model_data_fish()

message("Final fish model structure:")
for (resp in names(models_fish)) {
  message("  ", resp,
          " - year term: ",   if (model_has_term(models_fish[[resp]], "year"))   "yes" else "no",
          "; status term: ",  if (model_has_term(models_fish[[resp]], "status")) "yes" else "no")
}

# Guard - year and status are forced into every fish model as null.terms in
# 06_model-data_fish.R, so both should always be present here too.
if (!all(vapply(models_fish, model_has_term, logical(1), tm = "year")) ||
    !all(vapply(models_fish, model_has_term, logical(1), tm = "status"))) {
  stop("A final fish model is missing the forced year/status term. ",
       "Re-run 01_fit-final-models.R.")
}

# Importance heatmap - richness/abundance/cti share one FSS run (maxn), b20
# has its own (length)
fish_var_imp <- dplyr::bind_rows(
...
fish_signs <- purrr::map_dfr(names(models_fish), function(resp) {
  d <- dplyr::filter(fish_dat, response == resp)
  purrr::map_dfr(fish_term_order, function(tm) {
    tibble(response = resp, term = tm, sgn = term_sign(d$count, d[[tm]]))
  })
})

fig_c1_2 <- build_importance_plot(
  var_imp  = fish_var_imp,
  signs    = fish_signs,
  models   = models_fish,
...
ggsave(file.path(figdir, paste0(name, "_fish-importance.png")), fig_c1_2,
       width = 7, height = 6, dpi = 300, bg = "white")

# Response curves - year and status are forced parametric terms rather than
# by-year smooths, so the curve shape is the same every year and only its
# level shifts. Still built and shown per year, to match the habitat section.
fish_ncol <- grid_ncol(models_fish, function(resp) dplyr::filter(fish_dat, response == resp))

fish_ylim <- function(resp) range(dplyr::filter(fish_dat, response == resp)$count, na.rm = TRUE)

fish_curve_years <- levels(fish_dat$year)

for (yr in fish_curve_years) {

  message("Building fish response curves for: ", yr)

  fig_yr <- build_curve_grid(
    models         = models_fish,
    data_for       = function(resp) dplyr::filter(fish_dat, response == resp, as.character(year) == yr),
    obs_for        = function(resp) dplyr::filter(fish_dat, response == resp, as.character(year) == yr)$count,
    response_order = fish_response_order,
    term_order     = fish_term_order,
    ncol           = fish_ncol,
    pad_rows       = TRUE,
