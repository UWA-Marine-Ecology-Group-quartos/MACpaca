controlplot_benthos <- function(data, taxa, amp_abbrv, state_abbrv,
  ...
  plot_dat <- data %>%
    ...
    dplyr::mutate(
      year = as.numeric(as.character(year)), # scale_x_continuous() below needs numeric
  ...
  p <- ggplot(
    ...
    theme_classic() +
    scale_x_continuous(breaks = c(2014, 2021, 2024)) + # TODO set to your survey years
