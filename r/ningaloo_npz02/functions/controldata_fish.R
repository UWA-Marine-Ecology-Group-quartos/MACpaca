controldata_fish <- function(dat, year, amp_abbrv, state_abbrv) {
  ...
  marine_parks <- st_read("data/south-west network/spatial/shapefiles/western-australia_marine-parks-all.shp") %>%
    CheckEM::clean_names() %>%
    dplyr::mutate(
      # Computed from the raw zone (Sanctuary Zone or National Park Zone,
      # State or Commonwealth) before zone_new collapses State zones below -
      # keeps No-Take/Fished correct regardless of how zone_new is labelled
      status = ifelse(
        str_detect(zone, "Sanctuary Zone") | str_detect(zone, "National Park Zone"),
        "No-Take", "Fished"
      ),
      zone_new = case_when(
        ...
        # Everything else (State marine park zones, unmatched zone types, or
        # outside any marine park polygon) is just Coastal waters
        TRUE ~ "Coastal waters"
      )
    )
