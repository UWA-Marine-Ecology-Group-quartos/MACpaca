...
# National Park Zone ----
# Same widened NPZ window as 02_spatial-layers.R - the benthos models are
# fitted to samples inside this window only
...
benthos_all <- readRDS(paste0("data/", park, "/raw/", name, "_benthos.RDS")) %>%
  # Keep only the survey years listed in 00_config.yml.
...
# TODO Check this matches the NPZ window sample count reported by 02_spatial-layers.R
message("Benthos samples inside the NPZ window: ", nrow(benthos_raw), " of ",
        nrow(benthos_all))
...
if (nrow(benthos_raw) == 0) {
  stop("No benthos samples fall inside the NPZ window - check the shapefile ",
...
# on a fish. total_pts is therefore rebuilt from the five modelled classes so
