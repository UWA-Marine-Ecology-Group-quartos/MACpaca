...
# Spatial predictions limits - zoomed to the No-Take samples' bounding box +
# 0.1 degree buffer, same as 07 (benthos)
...
#   data/<park>/spatial/oceanography/SST-recent.nc
...
control_all <- purrr::map(years, \(yy) {
  ...
  controldata_fish(dat = dat_yy, year = yy, amp_abbrv = "NMP", state_abbrv = "NMP (State)")
})
...
# (dropped the "mixed" 2-year variant from the swc_westernarm template - it
# only makes sense for exactly 2 years, this appendix has 4)
# -------------------------------------------------------------------------
