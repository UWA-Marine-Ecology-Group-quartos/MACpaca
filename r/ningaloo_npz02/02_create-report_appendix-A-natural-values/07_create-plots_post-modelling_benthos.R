...
# Cropping extent - zoomed to the No-Take samples' bounding box + 0.1 degree
# buffer (113.5064-113.5832 lon, -22.7831 to -22.6669 lat before padding)
e <- ext(113.4064, 113.6832, -22.8831, -22.5669)
...
# Only sand and sessile invertebrates survived 05's >80%-zeros filter -
# macroalgae, seagrass and rock are not modelled, so their p_*.fit/se.fit
# layers don't exist in the predicted-habitat rds files
habitat_lookup <- c(
  "Sand" = "sand",
...
# Plot extent - zoomed to the No-Take samples' bounding box + 0.1 degree buffer
prediction_limits <- c(113.4064, 113.6832, -22.8831, -22.5669)
...
control_all <- purrr::map(years, \(yy) {
  ...
  # TODO check these park abbreviations - Ningaloo Marine Park is jointly
  # Commonwealth and State managed, so pick whatever short forms your report
  # style uses elsewhere
  controldata_benthos(dat = dat_yy, year = yy, amp_abbrv = "NMP", state_abbrv = "NMP (State)")
})
...
# Taxa to plot - only sand and sessile invertebrates are modelled (see
# habitat_lookup note above)
taxa_lookup <- c(
  "sand"       = "Sand",
...
# Extent of the study - zoomed to the No-Take samples' bounding box + 0.1
# degree buffer, same as the cropping extent above
e <- ext(113.4064, 113.6832, -22.8831, -22.5669)
...
hab_fills <- scale_fill_manual(
  name = "",
  limits = c("Rock", "Sessile invertebrates", "Sand"),
  values = c(
    "Rock" = "grey40",
    "Sessile invertebrates" = "plum",
    "Sand" = "wheat"
...
site_limits <- c(113.4064, 113.6832, -22.8831, -22.5669) # zoomed to the No-Take samples' bounding box + 0.1 degree buffer
