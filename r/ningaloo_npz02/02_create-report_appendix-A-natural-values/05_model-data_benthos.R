...
habi %>% dplyr::count(campaignid, year, status) %>% print(n = Inf)
...
# NPZ appendix - each survey year is modelled separately (status and year are
# both forced in below), so year is never a competing factor.vars here
...
# >80% zeros in the response-vector check above, so they never entered the
# FSS loop (unique.vars.use only has sand, sessile_invertebrates, reef).
...
# TODO If combine_benthos is switched to TRUE, remove `year +` and `, by = year`
if (combine_benthos &&
...
# TODO Zero here means zone_type does not use this string, and the whole
...
# Rock, macroalgae and seagrass are not modelled (see note above) so they are
# left out here
...
for (this_label in pred.labels) {
  ...
  # ---------------------------
  # Rock, macroalgae and seagrass are not modelled (see note above) so they
  # are left out here
  ...
  pred_dom_df <- as.data.frame(dom_rast, xy = TRUE, na.rm = TRUE) %>%
    dplyr::mutate(
      ...
      dom_tag = factor(
        dom_tag,
        levels = c("Sand", "Sessile invertebrates", "Reef")
      )
    )
