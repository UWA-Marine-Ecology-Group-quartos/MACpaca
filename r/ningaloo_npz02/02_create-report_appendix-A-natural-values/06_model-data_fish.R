...
# widen npz_pad_ns in 02/03/04 if this only shows one status
...
# Final models below match the simplest model within delta AICc <= 2 from
# the FSS output (ningaloo_npz02_all.mod.fits.csv / _b20_all.mod.fits.csv).
# status (and year) are kept in every model regardless of what selection returns.
# Family is tw() throughout, matching the Model1 test fit used in both FSS
# loops above.

# Total abundance - depth+reef (delta 0, wi 0.336), simplest of four models
# within delta AICc 2.
...
# Species richness - aspect+depth+reef (delta 0, wi 0.726), simplest of two
...
# CTI - depth alone (delta 0.063, wi 0.222), simplest of four models within
# delta AICc 2.
...
# B20 - aspect+depth+reef (delta 0, wi 0.54), simplest of two models within
# delta AICc 2.
...
# This appendix has combine_benthos = FALSE, so each fish year gets its own
# per-year reef surface from 05.
if (combine_benthos) {
...
stopifnot(!any(is.na(preddf_sy$year)))
## ------------------------------------------------------------
## PREDICT FISH METRICS FOR EVERY YEAR
## ------------------------------------------------------------
...
## ------------------------------------------------------------
## RASTERISE FISH PREDICTIONS BY YEAR (same format as habitat) - diagnostic
## check only, one quick plot per year
## ------------------------------------------------------------
for (yr in fish_years) {
  prasts_yr <- rast(
    predicted_fish %>%
      dplyr::filter(as.character(year) %in% as.character(yr)) %>%
      dplyr::select(x, y, starts_with("p_")),
    crs = "epsg:4326"
  )
  plot(prasts_yr)
  message(yr, " summary:")
  print(summary(prasts_yr))
}
