...
# National Park Zone ----
# Same widened NPZ window as 02/03/05 - fish metrics are built from samples
...
metadata_fish_all <- metadata %>%
  dplyr::filter(method %in% "BRUV") %>%
  dplyr::filter(as.character(year) %in% fish_years) %>%
  dplyr::select(-method)

metadata_fish <- in_npz(metadata_fish_all)

message("Metadata samples: ", nrow(metadata),
        " total | ", nrow(metadata_fish_all), " BRUV in fish_years | ",
        nrow(metadata_fish), " BRUV inside the NPZ window")

if (nrow(metadata_fish) == 0) {
  stop("No BRUV samples fall inside the NPZ window - check the shapefile and ",
       "the sample coordinates.")
}

# This is formatted habitat from 03_create-metrics_benthos
...
# -------------------------------------------------------------------------
# Commonwealth-only copy of metadata (already NPZ-window restricted, since
# metadata_fish above is built from in_npz())
# -------------------------------------------------------------------------
...
# TODO Check which parks the BRUVs landed in - should be almost entirely the
