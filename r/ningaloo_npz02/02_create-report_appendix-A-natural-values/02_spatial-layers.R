...
# Ningaloo straddles 114E - check the sample longitudes and pick whichever
# zone covers most of them
buffer_crs <- 32749
...
# National Park Zone ----
# This appendix is the NPZ-only variant. Rather than clip strictly to the NPZ
# polygon (which would leave every sample No-Take and give the "status" term
# nothing to contrast), the NPZ's bounding box is widened a little to the
# north and south so some Fished habitat outside the NPZ is picked up too.
# East-west is left alone - the NPZ is a narrow shore-parallel strip, so
# widening east-west would pull in a disproportionate amount of extra area.
# TODO "a little" - adjust if this pulls in too much/too little Fished area
npz_pad_ns <- 0.1

# TODO Check the shapefile path matches the network you are working in
npz <- st_read("data/north-west network/spatial/shapefiles/north-west-network-australia_marine-parks-all.shp") %>%
...
if (length(npz) == 0) {
  stop("No National Park Zone polygons returned - check the `zone_type` labels ",
       "in the marine parks shapefile.")
}

npz_bb <- st_bbox(npz)

npz_window <- st_as_sfc(st_bbox(c(
  xmin = npz_bb[["xmin"]], xmax = npz_bb[["xmax"]],
  ymin = npz_bb[["ymin"]] - npz_pad_ns, ymax = npz_bb[["ymax"]] + npz_pad_ns
), crs = 4326))

# Keep only samples that fall inside the widened NPZ window
in_npz <- function(df) {
  df %>%
    st_as_sf(coords = c("longitude_dd", "latitude_dd"), crs = 4326, remove = FALSE) %>%
    st_filter(npz_window, .predicate = st_within) %>%
    st_drop_geometry()
}
...
# TODO Check how many samples survived the NPZ window filter
message("Samples inside the NPZ window: ", nrow(metadata), " of ", nrow(metadata_all),
        " | BRUVs inside the NPZ window: ", nrow(metadata_bruv), " of ",
        nrow(metadata_bruv_all))

if (nrow(metadata) == 0 || nrow(metadata_bruv) == 0) {
  stop("No samples fall inside the NPZ window - check the shapefile and the ",
       "sample coordinates before going any further.")
}

# TODO Check both status levels came through - if this is only No-Take,
# widen npz_pad_ns above
...
make_buffer <- function(x) {
  x %>%
    ...
    st_transform(4326) %>%
    st_as_sf() %>%
    # Clip to the widened NPZ window - the 10 km buffer only ever removes
    # area from the window, it never extends the prediction surface beyond it
    st_intersection(npz_window) %>%
    st_make_valid()
}

sample_buffer <- make_buffer(metadata_habitat_sf)       # benthos - habitat samples
bruv_buffer   <- make_buffer(metadata_bruv_habitat_sf)  # fish    - BRUV habitat samples (unused this appendix, kept for parity)
...
# 10 km is retained around every sample where the NPZ window allows it.
