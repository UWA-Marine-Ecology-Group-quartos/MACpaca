...
metadata <- readRDS(paste0("data/", park, "/raw/metadata.RDS")) %>%
  dplyr::select(campaignid, sample, longitude_dd, latitude_dd, status, year, method) %>%
  glimpse()
...
# TODO Check which samples were dropped - the 250 m bathymetry has gaps around
# the islands, where most of the BOSS drops sit. The buffer itself should drop
# nothing, so anything here is still the bathymetry gaps.
metadata %>%
  anti_join(metadata.bathy.derivatives, by = c("campaignid", "sample")) %>%
  dplyr::count(method, year)

# Sampled depth range vs the range being predicted onto - predictions outside
# the sampled range are extrapolation
range(metadata.bathy.derivatives$geoscience_depth)
range(values(preds_benthos[["geoscience_depth"]]), na.rm = TRUE)
range(values(preds_fish[["geoscience_depth"]]), na.rm = TRUE)
