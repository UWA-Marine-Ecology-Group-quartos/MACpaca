...
# metadata. Two things go wrong otherwise:
...
count.wide <- count %>%
  ...
  # Year comes from the metadata rather than a campaignid pattern - the western
  # arm has two campaigns in 2020 (2020-06 and 2020-10), so a grepl on the
  # campaignid prefix would either split one year in two or need a rule per
  # campaign. The join also carries status, and is keyed on campaignid AND
  # sample because sample names repeat across campaigns.
  left_join(
    metadata_fish %>%
...
# The hardcoded park name list is gone - st_within below already restricts this
# to samples that fall inside a Commonwealth park, so naming the parks up front
# only creates a way for the filter to be wrong. Ngari Capes was in the old list
# but is a state park and never survived the epbc filter anyway.
...
# TODO Check which parks the BRUVs landed in - the western arm should be almost
# entirely South-west Corner
metadata_amp %>%
