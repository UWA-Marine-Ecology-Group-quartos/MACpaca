...
# want them reported some other way. (Benthos is pooled for this park, so
# `year` is not in the habitat FSS output at all.)
...
# s(x, by = f) reports `x` as term[1] and `f` as the by variable; the by
# variable is picked up separately from the parametric terms. No habitat model
# uses `by =` here (benthos is pooled), but the fish models carry `year` and
# `status` as parametric terms and those must survive into the set.
...
tidy_fss_csv <- function(path, response_in_column, expected = NULL) {
  ...
  # The two scripts drop a different column on the way out:
  #   05 writes select(all.mod.fits, -any_of(c("formula", "form"))), so
  #      `modname` survives and the formula is gone;
  #   06 writes all.mod.fits[ , -2], which assumes column 1 is `modname` and
  #      column 2 the long formula string - true there, because it uses
  #      do.call("rbind", ...) rather than list_rbind(names_to = "response").
  # Either way one of the two is present, so take modname when it is there and
  # rebuild it from the formula when it is not.
