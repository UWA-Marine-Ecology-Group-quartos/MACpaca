...
#
# THIS PARK: benthos is pooled, so habitat Model strings carry no factor term
# and top out at "detrended+roughness+aspect+Z". The habitat model column is
# left at the wider Geographe fraction anyway - it just leaves a little more
# whitespace, and nothing overruns. Fish strings do carry year/status. If a
# Model string wraps awkwardly, take another 0.02 off r2 and give it to model -
# just keep each set summing to 1.
...
# groups are several rows deep. Worth switching on for C 1.1 too if the habitat
