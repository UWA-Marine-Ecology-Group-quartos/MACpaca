...
# This script only DEFINES things - sourcing it is safe and side-effect free
# apart from reading the config. It is sourced by 03_create-figures.R and by
# 04_quarto.qmd.
#
# TWO SOURCES, both read-only:
#   1. FSS candidate tables written by 05/06 into
#        output/model-output/<park>/habitat/<name>_abiotic_all.mod.fits.csv
#        output/model-output/<park>/fish/maxn/<name>_all.mod.fits.csv
#        output/model-output/<park>/fish/length/<name>_b20_all.mod.fits.csv
#   2. The re-fitted final models saved by 01_ into
#        output/model-output/<park>/appendix-C/
#
# CSV LAYOUT DIFFERENCE:
#   - Habitat uses list_rbind(out.all, names_to = "response") -> `response` is
#     an explicit column.
#   - Fish uses do.call("rbind", out.all) on a named list -> the response is
#     embedded in the row names as "<response>.<n>", which write.csv dumps into
#     the first (unnamed) column.
# =============================================================================
...
habitat_term_order <- c("geoscience_detrended", "geoscience_roughness", "geoscience_depth")
fish_term_order    <- c("geoscience_detrended", "geoscience_roughness", "geoscience_depth", "reef")

# Row order in Table C 1.1 / C 2.1 and on the heatmaps. Drop "reef" from the
# habitat vector if you would rather Table C 1.1 only reported the three
# observed habitat classes.
habitat_response_order <- c("macroalgae", "sand", "sessile_invertebrates", "reef")
...
# Pretty "detrended+roughness+Z" string for the Model column
  ...
  purrr::imap_dfr(final_models, function(mod, resp) {
    ...
    if (!any(rows$selected)) {
      warning("The final model for '", resp, "' (", paste(final_set, collapse = "+"),
              ...
              "Check that 01_fit-final-models.R still matches 05/06.")
    }
