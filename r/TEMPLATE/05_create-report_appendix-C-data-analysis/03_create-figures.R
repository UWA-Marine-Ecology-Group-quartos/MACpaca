...
# Task:    Build Appendix C figures - importance heatmaps (C.1, C.2) and
#          GAM response-curve panels (C.1.n, C.2.n)
# Author:  Annika Leunig
# Date:    August 2026
###
# TODO If combine_benthos is TRUE for this park, the habitat response-curve
# section below (per-year loop) does not apply - pool the years into a single
# figure instead, and drop the per-year figure numbering block in 04_quarto.qmd.
...
# =============================================================================
# Model selection will not necessarily return the same shape for every
# response - some smooths may lose `by = year`, and some metrics may drop
# year and/or status entirely. curve_data() already copes - it only fills the
# factors that appear in gam_predictor_set(model), and predict.gam ignores
# anything else. These messages exist so the mismatch is visible when the
# figures are rebuilt, rather than being discovered in a caption that claims a
...
ggsave(file.path(figdir, paste0(name, "_habitat-importance.png")), fig_c1_1,
       width = 7, height = 6, dpi = 300, bg = "white")     # TODO adjust height to the number of habitat responses
...
build_curve_grid <- function(models, data_for, obs_for, response_order,
  ...
  for (resp in response_order) {
    ...
    # per-year habitat panels use it so different survey years are directly
    # comparable
...
# TODO If combine_benthos is TRUE for this park, replace this per-year loop
# with a single pooled figure instead (one habitat_data()/habitat_obs() pair
# using every year at once), and drop the per-year figure-numbering block in
# 04_quarto.qmd - see the TODO at the top of this file.
...
# before someone asks why two years look identical.
