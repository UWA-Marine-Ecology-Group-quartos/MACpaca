...
e <- c(123.1, 124.0, -35.1, -33.9) # xmin, xmax, ymin, ymax
...
CheckEM::ga_api_all_data(synthesis_id = "70", # BRUV - fish and benthos
                         token = token,
...
# samples inside a National Park Zone are No-Take, everything else is Fished.
# NOTE this is Commonwealth zoning only - state sanctuary zones are also no-take
# and will come back as Fished. Add them below if the study area includes any.
library(sf)
sf_use_s2(FALSE)
...
# A sample sitting on a shared boundary can match two polygons, so keep the
# more protective zone and take one row per sample.
sample_zones <- metadata %>%
  ...
  dplyr::arrange(campaignid, method, sample, zone != "National Park Zone") %>%
  dplyr::distinct(campaignid, method, sample, .keep_all = TRUE)
...
# The levels must be spelt exactly "No-Take" and "Fished" - 05, 06 and 08 match
# on those strings
metadata <- metadata %>%
  dplyr::select(-any_of("status")) %>%
  left_join(sample_zones %>%
              dplyr::select(campaignid, method, sample, zone) %>%
              mutate(status = if_else(zone %in% "National Park Zone",
                                      "No-Take", "Fished")),
            by = c("campaignid", "method", "sample")) %>%
...
# directly with knitr::kable(). sample_zones was built above when setting the
# status.
