...
# Clio Bank is a single-year (2025) survey. This is the suffix script 06 wrote
# the prediction rasters with, and the suffix used on the plot filenames here.
...
marine_parks <- st_read("data/south-west network/spatial/shapefiles/western-australia_marine-parks-all.shp") %>%
  dplyr::filter(name %in% "Abrolhos") # TODO select relevant parks
...
# Read the single prediction surface
dat <- readRDS(
  paste0("output/model-output/", park, "/fish/",
         name, "_predicted-fish_", years, ".rds")
)
...
#
# TODO Download the IMOS 6-day SST product: "IMOS - SRS - SST - L3S - Single
# Sensor - 6 day - day and night time - Australia" from the AODN portal, cropped
# to the Clio Bank window, and save it as:
...
for (metric_name in names(fish_metric_lookup)) {
  ...
  # Only build plot if both prediction and SE layers exist
...
# -------------------------------------------------------------------
# Single survey year and a single status (all Fished, SPZ), so these carry no
# temporal trend and no zone contrast. Kept for consistency with other parks -
# decide whether they earn a place in the appendix.

control_all <- list(
  controldata_fish(
    dat = dat_list[[as.character(years)]],
    year = as.numeric(years),
    amp_abbrv = "ABMP"   # TODO confirm park abbreviation
  )
)
...
# TODO check sampling effort - expecting 72 samples, 2025, all Fished
metadata_amp %>% count(year, status)

# -----------------------------
# Species Accumulation Curves
# -----------------------------
# Single year and a single status, so there is one curve per panel. The year
# linetype and status colour/fill mappings from the template are dropped -
# with one level each they add a legend with nothing to distinguish.
...
sac_sample <- ggplot(
  ...
  aes(x = x, y = richness, colour = status, fill = status)
) +
  geom_ribbon(aes(ymin = richness - sd, ymax = richness + sd),
              alpha = 0.18, colour = NA) +
  geom_line(linewidth = 1.2) +
  scale_colour_manual(name = "Status", values = c("Fished" = "#b9e6fb")) +
  scale_fill_manual(name = "Status", values = c("Fished" = "#b9e6fb")) +
...
# Bold labels flag species unique to one year's top 10. With a single year
# there is nothing to contrast, so no species are bolded.
if (length(years) > 1) {
...
bar_maxn <- ggplot(
  maxn.10 %>%
    mutate(scientific_label = if_else(scientific %in% unique_species,
                                      paste0("**", scientific, "**"),
                                      scientific)),
  aes(x = reorder(scientific_label, maxn), y = maxn)
) +
...
ggsave(paste0("plots/", park, "/fish/", name, "_top_maxn_bar_plot.png"),
       plot = bar_maxn, height = 4, width = 7, dpi = 300, units = "in", bg = "white")
...
cti.10 <- maxn %>%
  ...
  summarise(
    ...
    se   = sd(count, na.rm = TRUE) / sqrt(dplyr::n()),
    .groups = "drop") %>%
  left_join(sti, by = "scientific", relationship = "many-to-many") %>%
...
# centering statistic and global limits for the fill scale
mid_niche    <- median(cti.10$rls_thermal_niche, na.rm = TRUE)
niche_limits <- range(cti.10$rls_thermal_niche, na.rm = TRUE)

# TODO the geom_text y position is set for the observed abundance range -
# check the niche labels sit clear of the bars and adjust if not
bar_cti <- ggplot(
  cti.10 %>%
    mutate(
  ...
  # centre GREY at the median thermal niche
...
ggsave(paste0("plots/", park, "/fish/", name, "_top_maxn_cti_bar_plot.png"),
       plot = bar_cti, height = 4, width = 7, dpi = 300, units = "in", bg = "white")
...
# B20
# -------------------------------------------------------------------------
# All 72 samples are Fished (SPZ), so there is no zone contrast to draw. The
# template's Fished/No-Take split and the mixed-year variant are both dropped -
# the Combined values are the only meaningful summary at this park.
...
bar_b20 <- ggplot(
  b20.10,
  aes(x = reorder(scientific_name, b20), y = b20)
) +
  geom_col(
    fill = "grey40",
    colour = "black",
