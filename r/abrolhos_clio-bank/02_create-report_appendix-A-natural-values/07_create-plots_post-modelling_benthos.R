...
# Abrolhos AMP is Commonwealth-only, so this returns an empty sf. The benthos
# plotting functions subset it by `zone`, so it has to stay an sf object with
# that column rather than NULL - an empty layer just draws nothing.
# NOTE this differs from 08, where the fish functions ARE NULL-aware.
...
# Map pretty benthic names to raster layer prefixes in dat.
# Rock and seagrass failed the >80% zeros rule in 05 and were never modelled,
# so they have no prediction layers. They still appear in the scatterpie below,
# which reads the raw counts rather than the model output.
habitat_lookup <- c(
  "Sand" = "sand",
  "Macroalgae" = "macro",
...
# -------------------------------------------------------------------
if (!combine_benthos && length(years) > 1) {
...
# -------------------------------------------------------------------
# PART 3: Individual benthic class plots
# -------------------------------------------------------------------
...
# Skipped at Clio Bank: a single survey year gives no temporal contrast, and a
# single status (all Fished, SPZ) gives no zone contrast, so these plots would
# be a single point per depth class. Restore for parks with multiple years by
# setting the park abbreviations and the taxa actually modelled in 05.
# -------------------------------------------------------------------
if (!combine_benthos && length(years) > 1) {
  ...
  control_all <- purrr::map(years, \(yy) {
    ...
    controldata_benthos(dat = dat_yy, year = as.numeric(yy),
                        amp_abbrv = "ABMP", state_abbrv = NULL) # TODO set park abbreviations
  })
  ...
  # Taxa to plot - must match habitat_lookup above, since controldata_benthos()
  # only produces columns for the classes that were modelled in 05
  taxa_lookup <- c(
    "macro"   = "Macroalgae",
    "sand"    = "Sand",
  ...
  for (taxa_code in names(taxa_lookup)) {
    ...
    p_taxa <- controlplot_benthos(
      ...
      taxa = taxa_code,
      amp_abbrv = "ABMP", # TODO set park abbreviation
...
benthos <- readRDS(
  ...
  left_join(metadata_bathy_derivatives,
            by = c("campaignid", "sample", "year", "status")) %>%
  arrange(desc(Sand))

# Seagrass is zero in every sample at Clio Bank, so it renders as a zero-width
# slice. Kept in so the legend documents that it was scored for and not found -
# drop it from both limits and values if you would rather not show it.
...
if (combine_benthos) {
  ...
  # Same overwrite trap as PART 2 - with one year this writes to the same
  # filename as the per-year loop above, so it is guarded.
  if (length(years) > 1) {
