...
bubble_plots <- function(dat,
  ...
  # Bubble plots can be zoomed in tighter than the prediction surface. Falls
  # back to prediction_limits when bubble_limits is not supplied.
  if (is.null(bubble_limits)) bubble_limits <- prediction_limits
  ...
  build_panel <- function(metric_response, legend_title) {
    ...
    # Shared breaks so the colour and size guides merge into one legend with
    # n_size_breaks keys, showing the full range of bubble sizes.
    size_breaks <- scales::breaks_pretty(n = n_size_breaks)(
      range(plot_dat$count, na.rm = TRUE)
    )
    size_breaks <- size_breaks[size_breaks >= min(plot_dat$count, na.rm = TRUE) &
                                 size_breaks <= max(plot_dat$count, na.rm = TRUE)]
  ...
  cowplot::plot_grid(
    combined_panels,
    marine_park_legend(wasanc = wasanc),
    ncol        = 1,
